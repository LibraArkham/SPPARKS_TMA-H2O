...
void DiagAldTMA::compute()
{
  ...
  if (siteflag) {
    ...
    sites[OAlaX2] = 0; sites[OAlaX2H2O] = 0; sites[OAlaXOH] = 0; sites[OAlaX] = 0;
  ...
  for (int i = 0; i < nlist; i++) {
    ...
    else if (which[i] == QCM) ivalue = 0*sites[VACANCY] + 16*sites[O] + 17*sites[OH] + 27*sites[Ala] + 89*sites[OHAlaX3] + 
                                      73*sites[OAlaX2] + 91*sites[OAlaX2H2O] + 75*sites[OAlaXOH] + 58*sites[OAlaX] + 
