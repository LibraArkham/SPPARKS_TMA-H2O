...
def run(filename:str, outfile:str, strict_distance:float, stric_z:float, facet:tuple, layers:int, supercell:tuple, element_dict=None):
    """
    将CIF文件转换为SPPARKS data.ald文件的主函数
    
    参数:
    filename: 输入的CIF文件路径
    outfile: 输出的SPPARKS data.ald文件路径
    strict_distance: 近邻计算的截断距离（埃）
    stric_z: 基底层高度
    facet: 晶面，如(0,0,1)
    layers: 层数
    supercell: 扩胞大小，如(10,10,1)
    element_dict: 元素对应的数字表示，默认为{'Al':3,'O':1}
    """
...
def cif2pos(cif_file, pos_file, facet, layers, supercell):
    ...
    io.write(pos_file, slab_sort, direct=True)

def surface(bulk_structure, facet, layers):
    """从bulk结构中切割指定晶面"""
    from ase.build import cut, surface
    return surface(bulk_structure, facet, layers)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='将CIF文件转换为SPPARKS data.ald文件')
    parser.add_argument('cif_file', help='输入的CIF文件路径')
    parser.add_argument('--output', '-o', default='data.ald', help='输出的SPPARKS data.ald文件路径')
    parser.add_argument('--facet', default='(0,0,1)', help='晶面，格式为(h,k,l)')
    parser.add_argument('--layers', type=int, default=5, help='层数')
    parser.add_argument('--supercell', default='(1,1,1)', help='扩胞大小，格式为(nx,ny,nz)')
    parser.add_argument('--sub-layers', type=int, default=1, help='基底层数')
    parser.add_argument('--cutoff', type=float, default=2.5, help='近邻计算的截断距离（埃）')
    
    args = parser.parse_args()
    
    # 解析facet参数
    facet_str = args.facet.strip('()').split(',')
    facet = tuple(int(x) for x in facet_str)
    
    # 解析supercell参数
    supercell_str = args.supercell.strip('()').split(',')
    supercell = tuple(int(x) for x in supercell_str)
    
    # 运行主函数
    run(args.cif_file, args.output, args.cutoff, args.sub_layers, facet, args.layers, supercell)